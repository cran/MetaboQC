...
QCcorrectionMultiPoly6<-function(LCdata) {
  pos=1;
  ...
  while(n<(Day+1)){
    ...
    while(m<(length(CompoundsList)+1)){
      lm<-lm(bb[,CompoundsList[m]]~poly(bb[,"Order"],6,raw=TRUE));
      cc[,paste0(CompoundsList[m],'QC')]<-QCregression6(coefficients(lm)[1],coefficients(lm)[2],coefficients(lm)[3],coefficients(lm)[4],coefficients(lm)[5],coefficients(lm)[6],coefficients(lm)[7],LCdata[,"Order"]);
      m<-m+1
    }
