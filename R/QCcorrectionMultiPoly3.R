...
QCcorrectionMultiPoly3<-function(LCdata) {
  pos=1;
  ...
  while(n<(Day+1)){
    ...
    while(m<(length(CompoundsList)+1)){
      lm<-lm(bb[,CompoundsList[m]]~poly(bb[,"Order"],3,raw=TRUE));
      cc[,paste0(CompoundsList[m],'QC')]<-QCregression(coefficients(lm)[1],coefficients(lm)[2],coefficients(lm)[3],coefficients(lm)[4],cc[,"Order"]);
      m<-m+1
    }
