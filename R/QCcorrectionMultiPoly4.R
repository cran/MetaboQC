...
QCcorrectionMultiPoly4<-function(LCdata) {
  pos=1;
  ...
  variables<-c("QC","Day")
    nDAY<-ddply(as.data.frame(LCdata),variables,summarise,nSamples=sum(!is.na("QC")));
  ...
  while(n<(Day+1)){
    ...
    while(m<(length(CompoundsList)+1)){
      lm<-lm(bb[,CompoundsList[m]]~poly(bb[,"Order"],4,raw=TRUE));
      cc[,paste0(CompoundsList[m],'QC')]<-QCregression4(coefficients(lm)[1],coefficients(lm)[2],coefficients(lm)[3],coefficients(lm)[4],coefficients(lm)[5],cc[,"Order"]);
      m<-m+1
    }
