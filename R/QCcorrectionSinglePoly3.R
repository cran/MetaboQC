...
QCcorrectionSinglePoly3<-function(LCdata) {
  globalenv()
  LCdata<-LCdata
  ...
  m=1;
    while(m<(length(CompoundsList)+1)){
      lm<-lm(LCdataQC[,CompoundsList[m]]~poly(LCdataQC[,"Order"],3,raw=TRUE));
      LCdatanew[,paste0(CompoundsList[m],'QC')]<-QCregression(coefficients(lm)[1],coefficients(lm)[2],coefficients(lm)[3],coefficients(lm)[4],LCdatanew[,"Order"]);
      m<-m+1
    }
