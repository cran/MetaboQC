...
QCcorrectionSinglePoly6<-function(LCdata) {
  globalenv()
  LCdata<-LCdata
  ...
  m=1;
    while(m<(length(CompoundsList)+1)){
      lm<-lm(LCdataQC[,CompoundsList[m]]~poly(LCdataQC[,"Order"],6,raw=TRUE));
      LCdatanew[,paste0(CompoundsList[m],'QC')]<-QCregression6(coefficients(lm)[1],coefficients(lm)[2],coefficients(lm)[3],coefficients(lm)[4],coefficients(lm)[5],coefficients(lm)[6],coefficients(lm)[7],LCdatanew[,"Order"]);
      m<-m+1
    }
